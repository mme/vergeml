...
def test_live_loader_meta(tmpdir):
    ...
    loader = LiveLoader(cache_dir, st)
...
def test_disk_loader_meta(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir)})
    loader = FileCachedLoader(cache_dir, st)
    _test_loader_meta(loader)
...
def test_live_loader_with_ops_meta(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir)})
    loader = LiveLoader(cache_dir, st, ops=[AppendStringOperation()], output=st)
    _test_loader_meta(loader)

def test_mem_out_loader_with_ops_meta(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir)})
    loader = MemoryCachedLoader(cache_dir, st, ops=[AppendStringOperation()], output=st)
    _test_loader_meta(loader)

def test_disk_out_loader_with_ops_meta(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir)})
    loader = FileCachedLoader(cache_dir, st, ops=[AppendStringOperation()], output=st)
    _test_loader_meta(loader)
...
def test_mem_loader_with_ops_meta(tmpdir):
    ...
    st = SourceTest({'samples-dir': str(tmpdir)})
    loader = MemoryCachedLoader(cache_dir, st)
    loader2 = LiveLoader(cache_dir, loader, ops=[AppendStringOperation()], output=st)
...
def test_live_loader_with_multiplier_ops(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    loader = LiveLoader(cache_dir, st, ops=[AugmentOperation(variants=2), AppendStringOperation()], output=st)
...
def test_mem_out_loader_with_multiplier_ops(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    loader = MemoryCachedLoader(cache_dir, st, ops=[AugmentOperation(variants=2), AppendStringOperation()], output=st)
    _test_loader_read_samples_x2(loader)
...
def test_disk_out_loader_with_multiplier_ops(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    loader = FileCachedLoader(cache_dir, st, ops=[AugmentOperation(variants=2), AppendStringOperation()], output=st)
    _test_loader_read_samples_x2(loader)
...
def test_mem_loader_with_multiplier_ops(tmpdir):
    ...
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    loader = MemoryCachedLoader(cache_dir, st)
    loader2 = LiveLoader(cache_dir, loader, ops=[AugmentOperation(variants=2), AppendStringOperation()], output=st)
