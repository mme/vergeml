...
def test_data_disk_loader_meta(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir)})
    data = Data(input=st, cache_dir=cache_dir, cache_input='disk')
    _test_data_meta(data)
...
def test_data_live_loader_with_ops_meta(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir)})
    data = Data(input=st, cache_dir=cache_dir, ops=[AppendStringOperation()], cache_input=False, cache_output=False)
    _test_data_meta(data)

def test_data_mem_out_loader_with_ops_meta(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir)})
    data = Data(input=st, cache_dir=cache_dir, ops=[AppendStringOperation()], cache_input=False, cache_output='mem')
    _test_data_meta(data)

def test_data_disk_out_loader_with_ops_meta(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir)})
    data = Data(input=st, cache_dir=cache_dir, ops=[AppendStringOperation()], cache_input=False, cache_output='disk')
    _test_data_meta(data)

def test_data_mem_loader_with_ops_meta(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir)})
    data = Data(input=st, cache_dir=cache_dir, ops=[AppendStringOperation()], cache_input='mem', cache_output=False)
    _test_data_meta(data)

def test_data_disk_loader_with_ops_meta(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir)})
    data = Data(input=st, cache_dir=cache_dir, ops=[AppendStringOperation()], cache_input='disk', cache_output=False)
    _test_data_meta(data)
...
def test_data_mem_loader_read_samples(tmpdir):
    ...
    data = Data(input=st, cache_dir=cache_dir, cache_input='mem', cache_output=False)
    _test_data_read_samples(data)

def test_data_live_loader_read_samples(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    data = Data(input=st, cache_dir=cache_dir, cache_input=False, cache_output=False)
    _test_data_read_samples(data)

def test_data_disk_loader_read_samples(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    data = Data(input=st, cache_dir=cache_dir, cache_input='disk', cache_output=False)
    _test_data_read_samples(data)
...
def test_data_live_loader_with_multiplier_ops(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    data = Data(input=st, cache_dir=cache_dir, ops=[AugmentOperation(variants=2)], cache_input=False, cache_output=False)
...
def test_data_mem_out_loader_with_multiplier_ops(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    data = Data(input=st, cache_dir=cache_dir, ops=[AugmentOperation(variants=2)], cache_input=False, cache_output='mem')
    _test_data_read_samples_transformed_x2(data)
...
def test_data_disk_out_loader_with_multiplier_ops(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    data = Data(input=st, cache_dir=cache_dir, ops=[AugmentOperation(variants=2)], cache_input=False, cache_output='disk')
    _test_data_read_samples_transformed_x2(data)
...
def test_data_mem_loader_with_multiplier_ops(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    data = Data(input=st, cache_dir=cache_dir, ops=[AugmentOperation(variants=2)], cache_input='mem', cache_output=False)
    _test_data_read_samples_transformed_x2(data)
...
def test_data_disk_loader_with_multiplier_ops(tmpdir):
    cache_dir = _prepare_dir(tmpdir)
    st = SourceTest({'samples-dir': str(tmpdir), 'test-split': 2, 'val-split': 2})
    data = Data(input=st, cache_dir=cache_dir, ops=[AugmentOperation(variants=2)], cache_input='disk', cache_output=False)
    _test_data_read_samples_transformed_x2(data)
